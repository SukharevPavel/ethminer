...
#if GROUP_SIZE % 4 != 0
#error "GROUP_SIZE has to be a multiple of 4"
#endif
...
#elif defined(cl_amd_media_ops) && GROUP_SIZE <= 64
#error "GROUP_SIZE <= 64 isn't supported by newer AMD drivers and GROUP_SIZE > 64 is required"
#endif
    ...
    )
{
    ...
    __local compute_hash_share sharebuf[GROUP_SIZE / 4];
#ifdef LEGACY
    __local uint buffer[GROUP_SIZE / 4];
#else
...
__kernel void ethash_calculate_dag_item(uint start, __global const uint16 *_Cache, __global uint16 *_DAG, uint isolate)
{
